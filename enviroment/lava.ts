export function Createlava(): void {
...
let waterTexture = new Texture("images/textures/lava.png", {wrap: 1})
waterMaterial.albedoTexture = waterTexture
waterMaterial.albedoColor = new Color4(9,2,3,4)
waterMaterial.metallic = 0.8
waterMaterial.roughness = 1
waterMaterial.reflectivityColor = new Color3(0,0,0)

let currentScroll = 0.5 // Current scrolling amount (move this value to scroll)
...
waterPlane.uvs = [
  ...
  water.addComponent(new Transform({
    position: new Vector3(20.5, 0.39, 36.39),
    rotation: Quaternion.Euler(90, 0, 300),
    scale: new Vector3(10.2,25,4)
  }))
  water.addComponentOrReplace(waterMaterial)
  engine.addEntity(water)

  const TILING_FACTOR = 16   // How much texture repeats
  const SCROLL_SPEED = 0.0005  // How fast water scrolls
