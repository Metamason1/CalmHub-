export class Scene {
  ...
  constructor() {
    ...
    this.venueCenter = new Vector3(
      this.center.x + 0,
      this.groundElevation +0,
      this.center.z
    )
    this.columnsCenter = new Vector3(
      this.venueCenter.x,
      this.venueCenter.y,
      this.venueCenter.z
    )
    this.screensCenter = new Vector3(
      this.venueCenter.x - 8,
      this.venueCenter.y + 2,
      this.venueCenter.z
    )
    ...
    for (let i = 0; i < radius.length; i++) {
      ...
      this.tablePositions.push(
        this.venueCenter.add(
          Vector3.Backward()
            .rotate(Quaternion.Euler(0, angle, 0))
            .multiplyByFloats(radius[i], radius[i], radius[i])
        )
      )
    }
  }
